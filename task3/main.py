...
from data_utils import build_tokenizer, build_embedding_matrix, SNLIDataset
...
class Instructor:
    def __init__(self, args):
        ...
        self.trainset = SNLIDataset(args.dataset_file['train'], tokenizer)
        self.testset = SNLIDataset(args.dataset_file['test'], tokenizer)
    ...
    def _train(self, criterion, optimizer, train_data_loader, val_data_loader):
        ...
        for i_epoch in range(self.args.epochs):
            ...
            for i_batch, batch in enumerate(train_data_loader):
                ...
                inputs1 = batch[0].to(self.args.device)
                inputs2 = batch[1].to(self.args.device)
                targets = batch[2].to(self.args.device)
    ...
    def _evaluate_acc_f1(self, data_loader):
        ...
        with torch.no_grad():
            for i_batch, t_batch in enumerate(data_loader):
                t_inputs1 = t_batch[0].to(self.args.device)
                t_inputs2 = t_batch[1].to(self.args.device)
                t_targets = t_batch[2].to(self.args.device)
                t_outputs = self.model(t_inputs1, t_inputs2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='ESIM', type=str)
    ...
    parser.add_argument('--lr', default=4e-4, type=float, help='try 5e-5, 2e-5 for BERT, 1e-3 for others')
    ...
    model_classes = {
        ...
        # ESIM Hyperparameters(https://arxiv.org/pdf/1609.06038v3.pdf):
        # optimizer=Adam default params
        # lr = 4e-4
        # batch_size = 32
        # embed_dim = hidden_dim = 300
        # dropout = 0.5
